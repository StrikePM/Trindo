...
router.get('/rankHammer', verifyUser, async (req, res) => {
    const conn = await getConnection();
    try {
        const data = await conn.execute(`
        SELECT * FROM rank_hammer ORDER BY alternatif_id`);
...
router.post('/rankHammer', verifyUser, isAdmin, async (req, res) => {
    const conn = await getConnection()
    ...
    try {
        ...
        const data = await conn.execute(`INSERT INTO rank_hammer VALUES(DEFAULT,?,?,?)`, [productId, productCategory, netFlow]);
        let statusCode = 200;
        let message = 'success';
        if (data[0] == 0) {
            statusCode = 400;
...
router.delete('/rankHammer', verifyUser, isAdmin, async (req, res) => {
    const conn = await getConnection()
    try {
        const data = await conn.execute(`DELETE FROM rank_hammer`)
        var statusCode = 200, message = 'success';
        if (data[0].affectedRows > 0) {
            const tableName = 'rank_hammer';
